...
// Get WPSP Version - Just for display purposes
function bpsWhatVersion() {
echo " ~ 1.2.0";
}

// WPSP Master htaccess File Editing - file checks and get contents for editor
function get_secure_htaccess() {
	$secure_htaccess_file = '/wp-content/plugins/wordpress-security-plugin/admin/htaccess/secure.htaccess';
	if (file_exists(ABSPATH . $secure_htaccess_file)) {
	$bpsString = file_get_contents(ABSPATH . $secure_htaccess_file);
	echo $bpsString;
	} else {
	_e('The secure.htaccess file either does not exist or is not named correctly. Check the /wp-content/plugins/wordpress-security-plugin/admin/htaccess/ folder to make sure the secure.htaccess file exists and is named secure.htaccess.');
	}
}

function get_default_htaccess() {
	$default_htaccess_file = '/wp-content/plugins/wordpress-security-plugin/admin/htaccess/default.htaccess';
	if (file_exists(ABSPATH . $default_htaccess_file)) {
	$bpsString = file_get_contents(ABSPATH . $default_htaccess_file);
	echo $bpsString;
	} else {
	_e('The default.htaccess file either does not exist or is not named correctly. Check the /wp-content/plugins/wordpress-security-plugin/admin/htaccess/ folder to make sure the default.htaccess file exists and is named default.htaccess.');
	}
}

function get_maintenance_htaccess() {
	$maintenance_htaccess_file = '/wp-content/plugins/wordpress-security-plugin/admin/htaccess/maintenance.htaccess';
	if (file_exists(ABSPATH . $maintenance_htaccess_file)) {
	$bpsString = file_get_contents(ABSPATH . $maintenance_htaccess_file);
	echo $bpsString;
	} else {
	_e('The maintenance.htaccess file either does not exist or is not named correctly. Check the /wp-content/plugins/wordpress-security-plugin/admin/htaccess/ folder to make sure the maintenance.htaccess file exists and is named maintenance.htaccess.');
	}
}

function get_wpadmin_htaccess() {
	$wpadmin_htaccess_file = '/wp-content/plugins/wordpress-security-plugin/admin/htaccess/wpadmin-secure.htaccess';
	if (file_exists(ABSPATH . $wpadmin_htaccess_file)) {
	$bpsString = file_get_contents(ABSPATH . $wpadmin_htaccess_file);
	echo $bpsString;
	} else {
	_e('The wpadmin-secure.htaccess file either does not exist or is not named correctly. Check the /wp-content/plugins/wordpress-security-plugin/admin/htaccess/ folder to make sure the wpadmin-secure.htaccess file exists and is named wpadmin-secure.htaccess.');
	}
}
...
function root_htaccess_status() {
	...
	_e('<font color="red">wp-config.php is NOT .htaccess protected by WPSP</font><br><br>');
	} else {
	if (file_exists($filename)) {
	_e('<font color="green"><strong>The .htaccess file that is activated in your root folder is:</strong></font><br>');
		print($section);
	if ($check_string == "17") { 
		_e('<font color="green"><strong><br><br>&radic; wp-config.php is .htaccess protected by WPSP<br>&radic; php.ini and php5.ini are .htaccess protected by WPSP</strong></font><br><br>');
	} else {
	_e('<font color="red"><br><br><strong>A WPSP .htaccess file was NOT found in your root folder or you have not activated BulletProof Mode for your Root folder yet, Default Mode is activated, Maintenance Mode is activated or the version of the WPSP htaccess file that you are using is not 1.2.0. Please read the Read Me hover Tooltip above.</strong></font><br><br>');
	_e('<font color="red"><strong>wp-config.php is NOT .htaccess protected by WPSP</strong></font><br><br>');
	}
	}
	}
}
...
function wpadmin_htaccess_status() {
	...
	_e('<font color="red"><strong><br><br>A valid WPSP .htaccess file was NOT found in your wp-admin folder. Either you have not activated BulletProof Mode for your wp-admin folder yet or the version of the wp-admin htaccess file that you are using is not 1.2.0. BulletProof Mode for the wp-admin folder MUST also be activated when you have BulletProof Mode activated for the Root folder. Please read the Read Me hover Tooltip above.</strong></font><br>');
	}
	}
	}
}
	
// Check if WPSP Deny ALL htaccess file is activated for the WPSP Master htaccess folder
function denyall_htaccess_status_master() {
$filename = ABSPATH . '/wp-content/plugins/wordpress-security-plugin/admin/htaccess/.htaccess';
	if (file_exists($filename)) {
    _e('<font color="green"><strong>&radic; Deny All protection activated for WPSP Master /htaccess folder</strong></font><br>');
	} else {
    _e('<font color="red"><strong>Deny All protection NOT activated for WPSP Master /htaccess folder</strong></font><br>');
	}
}
// Check if WPSP Deny ALL htaccess file is activated for the /wp-content/bps-backup folder
...
// General WordPress Security Plugin File Status Checking
function general_bps_file_checks() {
	...
	$filename = '/wp-content/plugins/wordpress-security-plugin/admin/htaccess/default.htaccess';
	...
	$filename = '/wp-content/plugins/wordpress-security-plugin/admin/htaccess/secure.htaccess';
	if (file_exists(ABSPATH . $filename)) {
	...
	$filename = '/wp-content/plugins/wordpress-security-plugin/admin/htaccess/maintenance.htaccess';
	if (file_exists(ABSPATH . $filename)) {
	...
	$filename = '/wp-content/plugins/wordpress-security-plugin/admin/htaccess/bp-maintenance.php';
	if (file_exists(ABSPATH . $filename)) {
	...
	$filename = '/wp-content/plugins/wordpress-security-plugin/admin/htaccess/bps-maintenance-values.php';
	if (file_exists(ABSPATH . $filename)) {
	...
	$filename = '/wp-content/plugins/wordpress-security-plugin/admin/htaccess/wpadmin-secure.htaccess';
	if (file_exists(ABSPATH . $filename)) {
	...
	if (file_exists(ABSPATH . $filename)) {
    _e('<font color="green">&radic; Your WPSP Master default.htaccess file is backed up</font><br>');
	} else {
    _e('<font color="red">Your WPSP Master default.htaccess file is NOT backed up yet</font><br>');
	}
	
	$filename = '/wp-content/bps-backup/master-backups/backup_secure.htaccess';
	if (file_exists(ABSPATH . $filename)) {
    _e('<font color="green">&radic; Your WPSP Master secure.htaccess file is backed up</font><br>');
	} else {
    _e('<font color="red">Your WPSP Master secure.htaccess file is NOT backed up yet</font><br>');
	}
	
	$filename = '/wp-content/bps-backup/master-backups/backup_wpadmin-secure.htaccess';
	if (file_exists(ABSPATH . $filename)) {
    _e('<font color="green">&radic; Your WPSP Master wpadmin-secure.htaccess file is backed up</font><br>');
	} else {
    _e('<font color="red">Your WPSP Master wpadmin-secure.htaccess file is NOT backed up yet</font><br>');
	}

	$filename = '/wp-content/bps-backup/master-backups/backup_maintenance.htaccess';
	if (file_exists(ABSPATH . $filename)) {
    _e('<font color="green">&radic; Your WPSP Master maintenance.htaccess file is backed up</font><br>');
	} else {
    _e('<font color="red">Your WPSP Master maintenance.htaccess file is NOT backed up yet</font><br>');
	}

	$filename = '/wp-content/bps-backup/master-backups/backup_bp-maintenance.php';
	if (file_exists(ABSPATH . $filename)) {
    _e('<font color="green">&radic; Your WPSP Master bp-maintenance.php file is backed up</font><br>');
	} else {
    _e('<font color="red">Your WPSP Master bp-maintenance.php file is NOT backed up yet</font><br>');
	}
	
	$filename = '/wp-content/bps-backup/master-backups/backup_bps-maintenance-values.php';
	if (file_exists(ABSPATH . $filename)) {
    _e('<font color="green">&radic; Your WPSP Master bps-maintenance-values.php file is backed up</font><br>');
	} else {
    _e('<font color="red">Your WPSP Master bps-maintenance-values.php file is NOT backed up yet</font><br>');
	}
}
...
// Backup and Restore page - WPSP Master .htaccess backup file checks
...
function bps_check_permalinks() {
	...
	_e('Permalinks Enabled: <font color="red"><strong>WARNING! Permalinks are NOT Enabled<br>Permalinks MUST be enabled for WPSP to function correctly</strong></font>'); 
	}
}
...
function bps_check_php_version() {
	...
	if (version_compare(PHP_VERSION, '5.0.0', '<')) {
    _e('<font color="red"><strong>WARNING! WPSP requires PHP5 to function correctly. Your PHP version is: ' . PHP_VERSION . '</strong></font><br>');
	}
}
...
function bps_check_php_version_error() {
	...
	if (version_compare(PHP_VERSION, '5.0.0', '<')) {
    _e('<font color="red"><strong>WARNING! WPSP requires at least PHP5 to function correctly. Your PHP version is: ' . PHP_VERSION . '</font></strong><br><strong><a href="http://www.escalateseo.com/aitpro-blog/1166/wordpress-security-plugin-plugin-support/wordpress-security-plugin-plugin-guide-bps-version-45/#wordpress-security-plugin-issues-problems" target="_blank">WPSP Guide - PHP5 Solution</a></strong><br><strong>The WPSP Guide will open in a new browser window. You will not be directed away from your WordPress Dashboard.</strong><br>');
	}
}
...
function bps_check_permalinks_error() {
	...
	_e('<br><font color="red"><strong>WARNING! Permalinks are NOT Enabled. Permalinks MUST be enabled for WPSP to function correctly</strong></font><br><strong><a href="http://www.escalateseo.com/aitpro-blog/2304/wordpress-tips-tricks-fixes/permalinks-wordpress-custom-permalinks-wordpress-best-wordpress-permalinks-structure/" target="_blank">WPSP Guide - Enabling Permalinks</a></strong><br><strong>The WPSP Guide will open in a new browser window. You will not be directed away from your WordPress Dashboard.</strong><br>'); 
	}
}
...
function bps_check_safemode() {
	...
	_e('<br><font color="red"><strong>WARNING! WPSP has detected that Safe Mode is set to On in your php.ini file.</strong></font><br><strong>If you see errors that WPSP was unable to automatically create the backup folders this is probably the reason why.</strong><br>To remove this message permanently click <strong><a href="http://www.escalateseo.com/aitpro-blog/2566/wordpress-security-plugin-plugin-support/wordpress-security-plugin-error-messages" target="_blank">here.</a></strong><br>');
	} else {
	_e('');
	}
	}
...
function bps_multsite_check_smode() {  
	if ( is_multisite() ) { 
	_e('<strong>WordPress Network (Multisite) Installation Detected</strong><br>Please read the Read Me help hover tooltip for an additional step required to set up WordPress Network sites.<br>');
	} else {
	_e('');
	}
}
...
// Check for WP readme.html file and if valid WPSP .htaccess file is activated
// Get contents of Root .htaccess file from 3-45 - if second "4" found w/offset "15" in string position 17 - good - else bad
...
// Check for WP /wp-admin/install.php file and if valid WPSP .htaccess file is activated
// Get contents of Root .htaccess file from 3-45 - if second "4" found w/offset "15" in string position 17 - good - else bad
...
// Check WPSP Pro Modules Status
function check_bps_pro_mod () {
	global $securityplugin_security;
	$filename_pro = 'wp-content/plugins/wordpress-security-plugin/admin/options-bps-pro-modules.php';
	if (file_exists(ABSPATH . $filename_pro)) {
	$section_pro = file_get_contents(ABSPATH . $filename, NULL, NULL, 5, 10);
	_e('<font color="green"><strong>&radic; WordPress Security Plugin Pro Modules are installed and activated.</strong></font><br>');
	var_dump($section_pro);
	} else {
	_e('<font color="black"><br>*WPSP Pro Modules are not installed</font><br>');
	}
}
